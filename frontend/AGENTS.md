...
- Settings should prioritize "Mi perfil" first, then owner-only administration; keep technical placeholder context out of the main screen unless it becomes actionable.
...
- Show a clear "Personalizado" indicator when `hasCustomPreference` or field-level custom preference flags are true.
- Do not render add-food controls for users without `foods:create`; hide them instead of showing disabled buttons.
